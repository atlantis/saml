...
class RubySamlTest < Minitest::Test
  describe "SamlMessage" do
    ...
    let(:saml_message) { Saml::SamlMessage.new }
    ...
    describe "Prevent Zlib bomb attack" do
      let(:bomb) { Base64.encode(Zlib::Deflate.deflate(bomb_data, 9)[2..-5]) }
      ...
      it "raises error when SAML Message exceed the allowed bytes" do
        assert_raises(Saml::ValidationError, "Encoded SAML Message exceeds #{Saml::Settings::DEFAULTS[:message_max_bytesize]} bytes, so was rejected") do
          saml_message = Saml::SamlMessage.new
      ...
      describe "with a custom setting for message_max_bytesize" do
        ...
        let(:settings) { Saml::Settings.new({ :message_max_bytesize => message_max_bytesize }) }

        it "uses the custom setting" do
          assert_raises(Saml::ValidationError, "Encoded SAML Message exceeds #{message_max_bytesize} bytes, so was rejected") do
            saml_message = Saml::SamlMessage.new
