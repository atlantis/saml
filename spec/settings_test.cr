...
class SettingsTest < Minitest::Test
  describe "Settings" do
    ...
    it "create settings from hash" do
      ...
      @settings = Saml::Settings.new(config)
    ...
    it "does not modify default security settings" do
      ...
      new_settings = Saml::Settings.new
    ...
    it "overrides only provided security attributes passing a second parameter" do
      ...
      @default_attributes = Saml::Settings::DEFAULTS

      @settings = Saml::Settings.new(config, true)
    ...
    describe "#get_idp_cert" do
      ...
      it "returns the certificate when it is valid" do
        ...
        assert @settings.get_idp_cert.is_a? OpenSSL::X509::Certificate
      end
    ...
    describe "#get_idp_cert_multi" do
      ...
      it "returns the hash with certificates when values were valid" do
        certificates = [crystal_saml_cert_text]
        @settings.idp_cert_multi = {
          :signing => certificates,
          :encryption => certificates,
        }

        assert @settings.get_idp_cert_multi.is_a? Hash
        assert @settings.get_idp_cert_multi[:signing].is_a? Array
        assert @settings.get_idp_cert_multi[:encryption].is_a? Array
        assert @settings.get_idp_cert_multi[:signing][0].is_a? OpenSSL::X509::Certificate
        assert @settings.get_idp_cert_multi[:encryption][0].is_a? OpenSSL::X509::Certificate
      end
    ...
    describe "#get_sp_cert" do
      ...
      it "returns the certificate when it is valid" do
        ...
        assert @settings.get_sp_cert.is_a? OpenSSL::X509::Certificate
      end
      ...
      it "raises an error if SP certificate expired and check_sp_cert_expiration enabled" do
        ...
        assert_raises(Saml::ValidationError) { @settings.get_sp_cert }
      end
    end
    ...
    describe "#get_sp_cert_new" do
      ...
      it "returns the certificate when it is valid" do
        @settings.certificate_new = crystal_saml_cert_text
        assert @settings.get_sp_cert_new.is_a? OpenSSL::X509::Certificate
      end
    ...
    describe "#get_sp_key" do
      ...
      it "returns the private key when it is valid" do
        ...
        assert @settings.get_sp_key.is_a? OpenSSL::PKey::RSA
      end
    ...
    describe "#get_fingerprint" do
      ...
      it "get the fingerprint value when there is a cert at the settings" do
        ...
        @settings.idp_cert = crystal_saml_cert_text
        fingerprint = @settings.get_fingerprint
        assert fingerprint.downcase == crystal_saml_cert_fingerprint.downcase
      end
