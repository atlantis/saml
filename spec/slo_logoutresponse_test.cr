...
class SloLogoutresponseTest < Minitest::Test
  describe "SloLogoutresponse" do
    ...
    it "create the deflated SAMLResponse URL parameter" do
      unauth_url = Saml::SloLogoutresponse.new.create(settings, logout_request.id)
    ...
    it "support additional params" do
      unauth_url = Saml::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :hello => nil })
      assert_match(/&hello=$/, unauth_url)

      unauth_url = Saml::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :foo => "bar" })
      assert_match(/&foo=bar$/, unauth_url)

      unauth_url = Saml::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :RelayState => "http://idp.example.com" })
      assert_match(/&RelayState=http%3A%2F%2Fidp.example.com$/, unauth_url)
    end

    it "RelayState cases" do
      unauth_url = Saml::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :RelayState => nil })
      assert !unauth_url.includes?("RelayState")

      unauth_url = Saml::SloLogoutresponse.new.create(settings, logout_request.id, nil, { :RelayState => "http://example.com" })
    ...
    it "set a custom successful logout message on the response" do
      unauth_url = Saml::SloLogoutresponse.new.create(settings, logout_request.id, "Custom Logout Message")
    ...
    it "set a custom logout message and an status on the response" do
      unauth_url = Saml::SloLogoutresponse.new.create(settings, nil, "Custom Logout Message", {}, "urn:oasis:names:tc:SAML:2.0:status:PartialLogout")
    ...
    describe "playgin with preix" do
      it "creates request with ID prefixed with default '_'" do
        request = Saml::SloLogoutresponse.new
    ...
    describe "signing with HTTP-POST binding" do
      ...
      it "doesn't sign through create_xml_document" do
        unauth_res = Saml::SloLogoutresponse.new
      ...
      it "create a signed logout response" do
        logout_request.settings = settings
        params = Saml::SloLogoutresponse.new.create_params(settings, logout_request.id, "Custom Logout Message")

        response_xml = Base64.decode(params["SAMLResponse"])
    ...
    describe "signing with HTTP-Redirect binding" do
      ...
      it "create a signature parameter with RSA_SHA1 and validate it" do
        ...
        params = Saml::SloLogoutresponse.new.create_params(settings, logout_request.id, "Custom Logout Message", :RelayState => "http://example.com")
        assert params["SAMLResponse"]
        assert params[:RelayState]
        assert params["Signature"]
        assert_equal params["SigAlg"], XMLSecurity::Document::RSA_SHA1

        query_string = "SAMLResponse=#{URL.encode(params["SAMLResponse"])}"
    ...
    describe "#manipulate response_id" do
      it "be able to modify the response id" do
        logoutresponse = Saml::SloLogoutresponse.new
